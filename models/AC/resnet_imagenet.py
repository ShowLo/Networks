...
class BasicBlock(nn.Module):
    ...
    def __init__(self, in_planes, planes, stride=1, deploy=False, gamma_init=None):
        super(BasicBlock, self).__init__()
        self.convBn1 = ACBlock(in_channels=in_planes, out_channels=planes, kernel_size=3, stride=stride, padding=1, deploy=deploy, gamma_init=gamma_init)
...
class Bottleneck(nn.Module):
    ...
    def __init__(self, in_planes, planes, stride=1, deploy=False, gamma_init=None):
        ...
        self.bn1 = nn.BatchNorm2d(num_features=planes)
        self.convBn2 = ACBlock(in_channels=planes, out_channels=planes, kernel_size=3, stride=stride, padding=1, deploy=deploy, gamma_init=gamma_init)
...
class ResNet(nn.Module):
    def __init__(self, block, num_blocks, num_classes=1000, width_multiplier=1.0, deploy=False, gamma_init=1/3):
        ...
        self.convBn1 = ACBlock(in_channels=3, out_channels=ensure_divisible(64*width_multiplier, divisor), kernel_size=7, 
                               stride=2, padding=3, deploy=deploy, gamma_init=gamma_init)
        ...
        self.layer1 = self._make_layer(block, ensure_divisible(64*width_multiplier, divisor), num_blocks[0], stride=1, 
                                       deploy=deploy, gamma_init=gamma_init)
    ...
    def _make_layer(self, block, planes, num_blocks, stride, deploy, gamma_init):
        strides = [stride] + [1]*(num_blocks-1)
        layers = []
        for stride in strides:
            layers.append(block(self.in_planes, planes, stride, deploy, gamma_init))
...
def ResNet18(args):
    return ResNet(BasicBlock, [2, 2, 2, 2], args.num_classes, args.width_multiplier, args.deploy)


def ResNet34(args):
    return ResNet(BasicBlock, [3, 4, 6, 3], args.num_classes, args.width_multiplier, args.deploy)


def ResNet50(args):
    return ResNet(Bottleneck, [3, 4, 6, 3], args.num_classes, args.width_multiplier, args.deploy)


def ResNet101(args):
    return ResNet(Bottleneck, [3, 4, 23, 3], args.num_classes, args.width_multiplier, args.deploy)


def ResNet152(args):
    return ResNet(Bottleneck, [3, 8, 36, 3], args.num_classes, args.width_multiplier, args.deploy)