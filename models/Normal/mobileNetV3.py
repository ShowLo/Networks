    ...
    def __init__(self, in_channels_num, exp_size, out_channels_num, kernel_size, stride, use_SE, NL, BN_momentum):
        ...
        if exp_size == in_channels_num:
            ...
            self.conv = nn.Sequential(
                ...
                #nn.Sequential(OrderedDict([('lastBN', nn.BatchNorm2d(num_features=out_channels_num))])) if self.use_residual else nn.BatchNorm2d(num_features=out_channels_num, momentum=BN_momentum)
            )
        else:
            # With expansion
            self.conv = nn.Sequential(
...
class MobileNetV3(nn.Module):
    ...
    def __init__(self, mode='small', classes_num=1000, input_size=224, width_multiplier=1.0, dropout=0.2, BN_momentum=0.1):
        ...
        # input layer
        input_channels_num = ensure_divisible(first_channels_num * width_multiplier, divisor)
        last_channels_num = ensure_divisible(last_channels_num * width_multiplier, divisor) if width_multiplier > 1 else last_channels_num
        ...
        for kernel_size, exp_size, out_channels_num, use_SE, NL, stride in configs:
            output_channels_num = ensure_divisible(out_channels_num * width_multiplier, divisor)
            exp_size = ensure_divisible(exp_size * width_multiplier, divisor)
        ...
        # the last stage
        last_stage_channels_num = ensure_divisible(exp_size * width_multiplier, divisor)
